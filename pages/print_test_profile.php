...
<head>
    ...
    <title>Doctor | Lab Test Report</title>
...
<body>
    <div class="container-fluid">
        ...
        <?php   
            ...
            $pat_id=decode($x);
            $rec=$a->select('total_patient cp INNER JOIN total_checkup ct ON(cp.p_id=ct.p_id)','cp.p_name,cp.contact,cp.address,ct.checkup_date',"cp.p_id=$pat_id AND checkup_id=$checkup_id",'');
    ...
    ?>
                ...
                <div class="row">
                 <div>
                    <h4 style="display:inline-block;font-size:17px;font-weight:700;margin-top: 5px;margin-bottom: 5px;">Name: </h4>
                    <h4 style="display:inline-block;font-size:16px;font-weight:500;margin-top: 5px;margin-bottom: 5px;"><?php echo $data['p_name']; ?></h4>
                </div>
                 <div>
                    <h4 style="display:inline-block;font-size:17px;font-weight:700;margin-top: 5px;margin-bottom: 5px;">Phone #: </h4>
                    <h4 style="display:inline-block;font-size:16px;font-weight:500;margin-top: 5px;margin-bottom: 5px;"><?php echo $data['contact']; ?></h4>
                </div>
                 <div>
                    <h4 style="display:inline-block;font-size:17px;font-weight:700;margin-top: 5px;margin-bottom: 5px;">Address: </h4>
                    <h4 style="display:inline-block;font-size:16px;font-weight:500;margin-top: 5px;margin-bottom: 5px;"><?php echo $data['address']; ?></h4>
                </div>
                <div>
                    <h4 style="display:inline-block;font-size:17px;font-weight:700;margin-top: 5px;margin-bottom: 5px;" >Date: </h4>
                    <h4 style="display:inline-block;font-size:16px;font-weight:500;margin-top: 5px;margin-bottom: 5px;"><?php echo $data['checkup_date']; ?></h4>
                </div>
               
               
                 
                <div class="pull-right noprint" style="margin-right: 16px;">
                   <button onclick="myFunction()" class="btn"><i class="fa fa-print"></i> Print</button>
                </div>
                </div>
     <?php  } ?>
                <!-- /.row -->
    ...
    <?php   
            ...
            $x=$_REQUEST['pd'];
            $pat_id=decode($x);
        $lab_data=$a->select('patient_inhouse_test','*',"p_id=$pat_id",'');
    ...
    ?>
     <tr >
        ...
        <?php
        $in_data=$a->select('patient_insdie_inhouse_test','*',"p_id=$pat_id AND test_id=$test_id",'');
